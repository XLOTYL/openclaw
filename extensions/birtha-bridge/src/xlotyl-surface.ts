...
export function readXlotylBridgeConfig(config: unknown): XlotylBridgeConfig {
  const root = config && typeof config === "object" ? (config as Record<string, unknown>) : {};
  const raw =
    root.xlotyl && typeof root.xlotyl === "object" ? (root.xlotyl as Record<string, unknown>) : {};
  return {
...
export function shouldRegisterXlotylSurface(config: unknown): boolean {
...
async function probeAgentPlatform(cfg: XlotylBridgeConfig): Promise<Record<string, unknown>> {
...
export async function buildXlotylSurfaceDocument(
  config: unknown,
...
export function registerXlotylSurfaceRoute(
  api: Pick<OpenClawPluginApi, "config" | "registerHttpRoute">,
): void {
